...
const maxInArray = (arr) => {
  // Solution code here...
  const maxNum = arr.reduce((max , value) => {
    if(max < value )
    {
      max = value ;
    }
...
const checkValues = (obj, value) => {
  // Solution code here...
  let check = false;
  Object.values(obj).forEach(objValue => {if(objValue === value) check=true;} );
  return check ;
};
...
const updateNumbers = (obj) => {
  // Solution code here...
  const results = Object.keys(obj).map(key => `${String(key)}: ${String(obj[key])}` );
  return results ;
};
...
const hasChildrenValues = (arr, character) => {
  let check = false ;
  arr.forEach(obj => {
    if(Object.values(obj).indexOf(character) !== -1)
      if(Object.keys(obj).indexOf('children') !== -1) check=true;
  } );
  return check ;
